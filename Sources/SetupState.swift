...
class SetupState: ObservableObject {
    ...
    @Published var extraProviders: Set<String> = []
    ...
    private func buildTOML() -> String {
        var lines: [String] = []
        lines.append("# NeuralClaw Configuration")
        lines.append("# Generated by NeuralClaw Setup Wizard")
        lines.append("")
        ...
        lines.append("require_biometric = \(requireBiometric)")
        lines.append("")
    ...
    // MARK: - Launch NeuralClaw

    /// Launch the installed NeuralClaw.app from /Applications
    func launchNeuralClaw() {
        let appURL = URL(fileURLWithPath: "/Applications/NeuralClaw.app")
        let config = NSWorkspace.OpenConfiguration()
        // Pass setup data as launch arguments so the app knows it was just installed
        config.arguments = [
            "--from-setup",
            "--provider", provider.rawValue,
            "--model", selectedModel,
            "--biometric", requireBiometric ? "true" : "false"
        ]
        NSWorkspace.shared.openApplication(at: appURL, configuration: config) { _, error in
            if let error {
                print("Failed to launch NeuralClaw: \(error.localizedDescription)")
            }
        }
    }
}
